...
def create_data_loaders(data_dir, batch_size, num_workers=0):
    ...
    data_transforms = {
        ...
        'val': transforms.Compose([  # 新增：验证集变换
            transforms.Resize(256),
    ...
    ) for x in ['train', 'val', 'test']}  # 更新：包含验证集
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs=100, patience=10, device='cpu'):
    ...
    for epoch in range(num_epochs):
        ...
        # 训练和验证阶段
        for phase in ['train', 'val', 'test']:  # 更新：增加验证和测试阶段
            if phase == 'train':
                model.train()
            else:
                model.eval()
            ...
            # 基于验证集保存最佳模型
            if phase == 'val' and epoch_acc > best_acc:  # 更新：使用验证集评估早停
                best_acc = epoch_acc
                best_epoch = epoch
                best_model_wts = copy.deepcopy(model.state_dict())
